...
r <- brick(r1,r3,r5,r7[[c(1:5,10)]], r11,r12, r13, r14, r15, r16)
...
#trainValidateData$Class <- trainValidateData$CaseID
...
ValdfAll <- ValdfAll[complete.cases(ValdfAll),]

incombn <- createDataPartition(y=ValdfAll$class, p =0.5, list=F)
TestdfAll <- ValdfAll[incombn,]
ValdfAll <- ValdfAll[-incombn,]
...
algorithmList <- c('xgbTree','knn', 'rf', 'svmRadialCost')
...
#modFit2 <- train(as.factor(class) ~ ., method = 'avNNet', data = TrdfAll, preProcess = c('scale', 'center'), trControl=control)
#modFit2
#varImp(modFit2, scale=F)

models <- caretList(as.factor(class)~., data=TrdfAll, trControl=control, methodList=algorithmList, preProcess = c('scale', 'center'))
...
preds <- lapply(models, function(x){predict(x, newdata=TestdfAll)})
Accuracystats <- sapply(preds, function(x){confusionMatrix(x, TestdfAll$class)$overall[1]})
names(Accuracystats ) <- names(models)


preds <- data.frame(preds, class=TestdfAll$class)
set.seed(126)
combModFit.gbm <- train(as.factor(class) ~ ., method = "gbm", data = preds, distribution = "multinomial")
combPred.gbm <- predict(combModFit.gbm, preds)
Accuracystats <- c(Accuracystats, Combination = confusionMatrix(combPred.gbm, TestdfAll$class)$overall[1])
...
predVlist<- lapply(models, function(x){predict(x, newdata=ValdfAll)})
validationstats <- sapply(predVlist, function(x){confusionMatrix(x, ValdfAll$class)$overall[1]})
names(validationstats) <- names(models)

predVDF <- data.frame(predVlist, class=ValdfAll$class)
combPredV <- predict(combModFit.gbm, predVDF)

validationstats <- c(validationstats, Combination = confusionMatrix(combPredV, ValdfAll$class)$overall[1])

print(cbind(data.frame(Accuracystats), data.frame(validationstats)), digits=3)


currdir <- getwd()
setwd("D:\\informalsettlements\\banglore\\classificationwork\\Novclassifications")
filenames_types <- paste('pred_', names(models), '.tif', sep="")
filenames_probs <- paste('pred_probs_', names(models), '.tif', sep="")

predrast_type <- NULL
predrast <- NULL

for (i in 1:length(models)){
  predrast_type <- raster::predict(r, models[[i]], file=filenames_types[i], overwrite=T, datatype="INT1U", factors = levels(predVDF$class))
  
  if(i==1){predrast <- predrast_type}
   else{predrast <- stack(predrast, predrast_type)}
  
}

names(predrast) <- names(models)

ef <- data.frame(coordinates(predrast), values(predrast[[1]]), values(predrast[[2]]), values(predrast[[3]]),values(predrast[[4]]))
ef[,3:6] <- lapply(ef[,3:6], function(x){factor(x, levels=1:5, labels=levels(predVDF$class))})
names(ef)[3:6] <- names(models)
ef <- ef[which(!is.na(ef[,3])),]
ef$gbmclass <- predict(combModFit.gbm, ef[,3:6])

e <- extent(predrast)
ef.r <- raster(e, ncol = ncol(predrast), nrow=nrow(predrast), crs(predrast))
sad <- rasterize(ef[,1:2], ef.r, as.numeric(ef$gbmclass), file="pred_combGBM.tif",overwrite=T)
...
# 
# 
# model_preds <- lapply(models, predict, newdata=ValdfAll, type="prob")
# model_preds <- lapply(model_preds, function(x) x[,"Informal"])
# model_preds <- data.frame(model_preds)
# ens_preds <- predict(greedy_ensemble, newdata=ValdfAll, type="prob")
# model_preds$ensemble <- ens_preds
# caTools::colAUC(model_preds, ValdfAll$class)
...
# beginCluster()
# predrast <- clusterR(r, raster::predict, args = list(model = greedy_ensemble, type="prob"))
# writeRaster(predrast, file="D:\\informalsettlements\\banglore\\classificationwork\\pred_greedy_nov_prob.tif", overwrite=T)
# predrast <- clusterR(r, raster::predict, args = list(model = greedy_ensemble))
# writeRaster(predrast, file="D:\\informalsettlements\\banglore\\classificationwork\\pred_greedy_nov.tif", overwrite=T, datatype="INT2S")
# 
# endCluster()



predrast_prob <- raster::predict(r, model=greedy_ensemble, type="prob", file="D:\\informalsettlements\\banglore\\classificationwork\\pred_greedy_nov_prob.tif", overwrite=T)
predrast_type <- raster::predict(r, model=greedy_ensemble, file="D:\\informalsettlements\\banglore\\classificationwork\\pred_greedy_nov.tif", overwrite=T)
